...
// 이메일 인증
// document.querySelector('button[name="btnVerify"]').addEventListener("click", function () {
...
// 		}
// 	};
// 	xhr.send(JSON.stringify({ user_email: user_id.value }));
// });
// 회원가입
function fnJoin() {
	...
	xhr.onreadystatechange = function () {
		if (this.readyState === XMLHttpRequest.DONE && this.status === 200) {
			...
			if (res.code === 1) {
				...
				alert("사용 중인 이메일 입니다. 다른 이메일을 등록해 주세요");
			} else if (res.code === "auth/weak-password") {
				alert("비밀번호는 최소 6 자리 이상으로 입력해주세요");
			} else {
				alert(res.message);
			}
		}
