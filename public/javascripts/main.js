$(function () {
  ...
  function fnGenerateGroundList(res, currentSlide){
    ...
    res.forEach(function(game, idx){
      html += '<li class="list-group-item list-group-item-light py-1 px-0 mx-auto ' + (idx%2===0?"bg-light":"") + '" style="border:none !important">' +
        '<div class="row w-100">' +
        '<div class="col-4 col-md-2 d-block">' +
        '<small class="font-weight-bold text-secondary">TIME</small>' +
        '<h5 class="text-orange">' + game.label_time + '</h5>' +
        '</div>' +
        '<div class="col-8 col-md-7 text-left">' +
        '<small class="text-secondary font-weight-bold">' + game.label_title + '</small>' +
        ...
        '<small class="pl-2 tagList male text-primary">' + game.label_sex + '</small>' +
        '</div>' +
        '<div class="grade_icon ml-2" data-grade="' + game.match_grade + '">실력</div>' +
        '<div class="ladder_icon ml-2">승점</div>' +
        '</div>' +
        '</div>' +
        '<div class="col col-md-3">' +
        '<button class="btn btn-status text-white p-0" data-status="' + game.apply_status + '" tabindex="0">' +
        '<h5 class="m-0 py-1">' + (game.apply_status === "hurry" ? "마감임박" : game.apply_status === "available" ? "신청가능" : "마  감") + '</h5>' +
        '<div class="text-warning bg-white mx-auto font-weight-bold" style="border-radius:1rem; width:100px; font-size:13px;">1자리 남음</div>' +
        '<small class="position-relative" style="top:-3px;">' + game.label_cash + '</small>' +
        '</button>' +
        '</div>' +
        '</div>' +
        '</li>';
    });
    document.querySelector('div[data-slick-index="' + currentSlide + '"] ul').innerHTML = html;
  }
});
//  날짜 선택 이벤트
