...
router.post("/register", async (req, res) => {
	...
	if (certified) {
		// firebase 이메일/비밀번호 등록
		firebase
			.auth()
			.createUserWithEmailAndPassword(req.body.user_id, req.body.user_pw)
			.then(async () => {
...
router.get("/mypage", middle.isSignedIn, async (req, res) => {
	...
	let user = await User.aggregate([
		...
		{ $unwind: { path: "$club_info", preserveNullAndEmptyArrays: true } },
		{
			$project: {
				...
				certified: 1,
				provider: 1,
				favorite_ground: { $ifNull: ["$favorite_ground", []] },
				club_info: { $ifNull: ["$club_info", ""] },
	...
	//  즐겨찾는 구장
	let favorite_match_query = user[0].favorite_ground.map((v) => {
...
router.post("/mypage/myinfo", middle.isSignedIn, async (req, res) => {
	try {
		let user_info = req.session.passport;
		// 패스워드를 변경할 경우 firebase 비밀번호도 함께 업데이트 시켜줘야 함.
		if (req.body.user_pw) {
			firebase.auth().currentUser.updatePassword(req.body.user_pw);
		}
		let result = await User.updateOne(
			{
...
router.post("/resetPw", async (req, res) => {
	try {
		let result = await User.updateOne({ oobCode: req.body.oobCode, user_pw: req.body.user_pw });
	} catch (err) {
		res.json({ code: 0, message: err.message });
	}
});
